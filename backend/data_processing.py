...
def inspect_dataset(csv_path: str) -> Dict[str, Any]:
    ...
    if not suggested_location and text_columns:
        # Pick text column with multiple non-unique strings or reasonable categories
    ...
    if not suggested_target and numeric_columns:
        # Pick a numeric column different from time
...
def clean_dynamic_data(
    ...
    if location_col and str(location_col).strip() and str(location_col).strip() != "None" and location_col in work.columns:
        ...
        if location_val and str(location_val).strip() and str(location_val).strip() != "ALL":
            ...
            # If multiple locations exist but none specified, ask user to select
            if location_val != "ALL":
                raise ValueError(
    ...
    # If time column is formatted like '2015-06-01' or dates, extract year or numeric timestamp
    ...
    if parsed_dates.notna().sum() > len(work) * 0.7:
        ...
        # If all years are whole years without specific dates, keep whole years
    ...
    # Assign standardized working columns for analysis while preserving original
...
def load_data(csv_path: str) -> pd.DataFrame:
    """Legacy loader with strict REQUIRED_COLUMNS check."""
    df = read_csv_safely(csv_path)
...
def get_segments(csv_path: str, location_col: str = "Segment") -> List[str]:
    """Legacy segment extractor."""
    try:
        df = read_csv_safely(csv_path)
        # Check requested location_col or fallback to auto-detected text column
...
def clean_data(df: pd.DataFrame, segment: Optional[str] = None) -> pd.DataFrame:
    """Legacy cleaner for Year, Segment, ShorelinePosition_m."""
    cleaned = clean_dynamic_data(
        df,
        time_col="Year",
        ...
        min_records=2  # legacy allowed 2
    )
