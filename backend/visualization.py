...
def plot_dynamic_rate(
    ...
    # Exclude baseline first step if rate is 0.0
    plot_df = history.iloc[1:] if len(history) > 1 else history

    times = plot_df[t_col].values
    rates = plot_df[rate_col].values

    # Determine colors: if target is position/width (loss is negative change or retreat), highlight accordingly
    # For general rate: positive is green (growth) or orange (retreat)
